...
final class ProductTypeServiceTest extends WireMockBaseTestCase {
  ...
  void search() throws URISyntaxException, IOException {
    ...
    stubFor(get(urlPathEqualTo("/api/v2/product_types/"))
  ...
  void get_byId() {
    final var response = """
      ...
      """;
    stubFor(get(urlPathEqualTo("/api/v2/product_types/1"))
      .willReturn(ok()
        .withHeaders(responseHeaders(response.length()))
        .withBody(response)
      ));
  ...
  void searchUnique_withSearchObject() throws URISyntaxException, JsonProcessingException {
    ...
    final var searchObject = ProductType.builder()
      .name("foo")
      .build();
  ...
  void create() {
    final var json = """
      ...
      """;
    stubFor(post(urlPathEqualTo("/api/v2/product_types/"))
  ...
  @Test
  void delete_byId() {
    stubFor(delete(urlPathEqualTo("/api/v2/product_types/42/"))
      .willReturn(ok()));

    sut.delete(42L);

    verify(deleteRequestedFor(urlPathEqualTo("/api/v2/product_types/42/")));
  }
  ...
  void update() {
    final var json = """
      ...
      """;
    stubFor(put(urlPathEqualTo("/api/v2/product_types/42/"))
      .withRequestBody(equalToJson(json))
      .willReturn(ok()
        .withHeaders(responseHeaders(json.length()))
        .withBody(json)
